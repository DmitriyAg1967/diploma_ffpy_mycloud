(function (global, factory) {
    ...
    // Пользовательская модальная коробка
    function Modal($elem) {
        this.elem = $elem
    ...
    // Определите, является ли это мобильным терминалом
    function isMobile() {
    ...
    function copyText(text, alert) {
        if (window.isSecureContext) {
            navigator.clipboard.writeText(text).then(() => {
                ...
                alert.setText('Копирование успешно')
            }, () => {
                alert.setIcon('fas fa-exclamation-circle')
                alert.setText('Ошибка копирования')
            }).finally(() => {
                alert.show()
            })
        } else {
    ...
    // Первоначальная информация для проверки
    ...
    // Восстановить проверочную информацию
    ...
    function checkPassword(password, confirm) {
        if (password.val() !== confirm.val()) {
            password.siblings('.invalid-feedback').text('Пароль дважды не соответствует действительности')
            confirm.siblings('.invalid-feedback').text('Пароль дважды не соответствует действительности')
